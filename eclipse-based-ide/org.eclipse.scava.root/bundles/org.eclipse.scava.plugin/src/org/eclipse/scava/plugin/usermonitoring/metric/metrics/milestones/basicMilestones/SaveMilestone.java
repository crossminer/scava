...
public class SaveMilestone implements IBasicMilestone, IHasRemainder, IProjectSpecificMilestone {
	...
	public SaveMilestone(GraphTraversal<Vertex, Vertex> graphTraversal, MetricRemainHandler remainHandler, String project, Date currentTime) {
	...
	public List<TimeintervalGraphtraversal<Vertex, Vertex>> getSubWindows() {
		...
		milestoneOccurences.addAll((Collection<? extends Vertex>) graphTraversal.asAdmin().clone().has("VertexType", VertexType.ELEMENT_EVENT).has("Type", "SAVED").as("event")
				.out(EdgeType.RELATED_PROJECT).has("ProjectId", project).select("event").order().by("TimeStamp", Order.incr).toList());
