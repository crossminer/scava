...
public class CalculationResultsSink extends CalculationResultsSinkBase {
	...
	public synchronized void consumeCalculationResults(CalculationResult result) throws Exception {
		if (writer == null) {
			...
			writer = new CsvWriter(output.getAbsolutePath(), "a", "operator", "b", "result", "worker", "workerLang");
		}

		writer.writeRecord(result.getA(), result.getOperator(), result.getB(), result.getResult(), result.getWorker(),
				result.getWorkerLang());
		writer.flush();
	}
