...
public class AdditionResults implements Channel {
	...
	public AdditionResults(Workflow workflow) throws Exception {
		...
		pre.put("AdditionResultsSink", (ActiveMQDestination) session.createQueue("AdditionResultsPre.AdditionResultsSink"));
		destination.put("AdditionResultsSink", (ActiveMQDestination) session.createQueue("AdditionResultsDestination.AdditionResultsSink"));
		post.put("AdditionResultsSink", (ActiveMQDestination) session.createQueue("AdditionResultsPost.AdditionResultsSink"));
						...
						if (workflow.isCacheEnabled() && workflow.getCache().hasCachedOutputs(job)) {
							for (Job output : workflow.getCache().getCachedOutputs(job)) {
								if (output.getDestination().equals("AdditionResults")) {
									...
									System.err.println(
											output.getDestination() + " destination found in AdditionResults pre consumer");
								}
							}
						} else {
