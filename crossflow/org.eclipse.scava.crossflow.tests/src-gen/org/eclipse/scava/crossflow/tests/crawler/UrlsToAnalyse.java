...
public class UrlsToAnalyse implements Channel {
	...
	public UrlsToAnalyse(Workflow workflow) throws Exception {
		...
		pre.put("UrlAnalyser", (ActiveMQDestination) session.createQueue("UrlsToAnalysePre.UrlAnalyser"));
		destination.put("UrlAnalyser", (ActiveMQDestination) session.createQueue("UrlsToAnalyseDestination.UrlAnalyser"));
		post.put("UrlAnalyser", (ActiveMQDestination) session.createQueue("UrlsToAnalysePost.UrlAnalyser"));
	...
	public void send(Url url, String taskId) {
		try {
			...
			if ((d = pre.get(taskId)) != null) {
				...
				url.setDestination("UrlsToAnalyse");
				message.setObject(url);
				producer.send(message);
				producer.close();
			} else
	...
	public void addConsumer(UrlsToAnalyseConsumer consumer, String consumerId) throws Exception {
