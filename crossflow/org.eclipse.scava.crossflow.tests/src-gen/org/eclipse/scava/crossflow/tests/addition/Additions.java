...
public class Additions implements Channel{
	...
	public void addConsumer(AdditionsConsumer consumer, String consumerId) throws Exception {
		...
		ActiveMQDestination preQueue = (ActiveMQDestination) session.createQueue("AdditionsPre." + consumerId);
		pre.put(consumerId, preQueue);	
	
		ActiveMQDestination destQueue = (ActiveMQDestination) session.createQueue("AdditionsDestination." + consumerId);
		destination.put(consumerId, destQueue);	
	
		ActiveMQDestination postQueue = (ActiveMQDestination) session.createQueue("AdditionsPost." + consumerId);
						...
						if (workflow.isCacheEnabled() && workflow.getCache().hasCachedOutputs(job)) {
							for (Job output : workflow.getCache().getCachedOutputs(job)) {
								if (output.getDestination().equals("Additions")) {
									((AdditionWorkflow) workflow).getAdditions().send((NumberPair) output, this.getClass().getName());
								}
								if (output.getDestination().equals("AdditionResults")) {
									((AdditionWorkflow) workflow).getAdditionResults().send((Number) output, this.getClass().getName());
								}
							}
						}
						else {
