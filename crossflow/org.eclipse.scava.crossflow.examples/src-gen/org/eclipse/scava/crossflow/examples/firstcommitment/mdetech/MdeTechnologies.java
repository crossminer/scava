...
public class MdeTechnologies implements Channel {
	...
	public MdeTechnologies(Workflow workflow) throws Exception {
		...
		pre.put("MdeTechnologyRepoFetcher", (ActiveMQDestination) session.createQueue("MdeTechnologiesPre.MdeTechnologyRepoFetcher"));
		destination.put("MdeTechnologyRepoFetcher", (ActiveMQDestination) session.createQueue("MdeTechnologiesDestination.MdeTechnologyRepoFetcher"));
		post.put("MdeTechnologyRepoFetcher", (ActiveMQDestination) session.createQueue("MdeTechnologiesPost.MdeTechnologyRepoFetcher"));
		
	}
	
	public void send(ExtensionKeywordTuple extensionKeywordTuple, String taskId) {
		try {
	...
	public void addConsumer(MdeTechnologiesConsumer consumer, String consumerId) throws Exception {
						...
						if (workflow.isCacheEnabled() && workflow.getCache().hasCachedOutputs(job)) {
							for (Job output : workflow.getCache().getCachedOutputs(job)) {
								if (output.getDestination().equals("MdeTechnologies")) {
									...
									System.err.println(
											output.getDestination() + " destination found in MdeTechnologies pre consumer");
								}
							}
						} else {
