...
public class GhRepos {
	...
	public GhRepos(Workflow workflow) throws Exception {
		...
		pre = session.createQueue("GhRepoUrlPre");
		post = session.createQueue("GhRepoUrlPost");
	...
	public void send(GhRepo ghRepo) {
		try {
	...
	public void addConsumer(GhReposConsumer consumer) throws Exception {
		...
		messageConsumer.setMessageListener(new MessageListener() {
			...
			public void onMessage(Message message) {
				...
				try {
					Job job = (Job) objectMessage.getObject();
					consumer.consumeGhRepo((GhRepo) job);
