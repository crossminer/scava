...
public class ResultsPublisher2 implements Channel{
	...
	public ResultsPublisher2(Workflow workflow) throws Exception {
	...
	public void send(Result result, String taskId) {
		try {
			...
			if ((d = pre.get(taskId)) != null) {
				...
				result.setDestination("ResultsPublisher2");
	...
	public void addConsumer(ResultsPublisher2Consumer consumer, String consumerId) throws Exception {

		// XXX use runtime class as ID of consumer as tasks are unique
	
		ActiveMQDestination preQueue = (ActiveMQDestination) session.createQueue("ResultsPublisher2Pre." + consumerId);
		pre.put(consumerId, preQueue);	
	
		ActiveMQDestination destQueue = (ActiveMQDestination) session.createQueue("ResultsPublisher2Destination." + consumerId);
		destination.put(consumerId, destQueue);	
	
		ActiveMQDestination postQueue = (ActiveMQDestination) session.createQueue("ResultsPublisher2Post." + consumerId);
						...
						if (workflow.isCacheEnabled() && workflow.getCache().hasCachedOutputs(job)) {
							for (Job output : workflow.getCache().getCachedOutputs(job)) {
								if (output.getDestination().equals("ResultsPublisher2")) {
									...
									System.err.println(
											output.getDestination() + " destination found in ResultsPublisher2 pre consumer");
								}
							}
						} else {
		...
		messageConsumer.setMessageListener(new MessageListener() {
			...
			public void onMessage(Message message) {
				...
				try {
					...
					consumer.consumeResultsPublisher2Actual(result);
