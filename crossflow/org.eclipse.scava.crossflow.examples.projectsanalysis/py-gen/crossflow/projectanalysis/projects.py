...
class Projects(JobStream):
    ...
    def __init__(self, workflow: Workflow, enable_prefetch: bool):
        super().__init__("Projects", workflow)

        # RepositoryCloner
        self._pre["RepositoryCloner"] = QueueInfo(
            f"ProjectsPre.RepositoryCloner.{self.workflow.instance}",
            False
        )
        self._destination['RepositoryCloner'] = QueueInfo(
            f"ProjectsDestination.RepositoryCloner.{self.workflow.instance}",
            False
        )
        self._post['RepositoryCloner'] = QueueInfo(
            f"ProjectsPost.RepositoryCloner.{self.workflow.instance}",
            False,
        ...
        if consumer is not None:
            def handler(message, ack_func=None):        
                try:
                    ...
                    consumer.consumeProjectsWithNotifications(obj)
