...
import crossflow.diagram.part.CrossflowLinkDescriptor;
...
public class WorkflowCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	*/
	protected void refreshSemantic() {
		...
		for (View v : getViewChildren()) {
			if (isShortcut(v)) {
				if (CrossflowDiagramUpdater.isShortcutOrphaned(v)) {
					orphaned.add(v);
				}
				continue;
			}
	...
	*/
	private Collection<CrossflowLinkDescriptor> collectAllLinks(View view, Domain2Notation domain2NotationMap) {
		...
		case ReusableComponentEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CrossflowDiagramUpdater.getReusableComponent_2017ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	*/
	protected final EditPart getHintedEditPart(EObject domainModelElement, Domain2Notation domain2NotationMap,
		...
		View view = (View) domain2NotationMap.getHinted(domainModelElement,
				CrossflowVisualIDRegistry.getType(hintVisualId));
		if (view != null) {
			return (EditPart) getHost().getViewer().getEditPartRegistry().get(view);
		}
		return null;
