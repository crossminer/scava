...
package org.eclipse.crossmeter.metricprovider.trans.newsgroups.threads;
...
//    }
    ...
    void flush() {
    	...
    	Stack<ThreadContainer> stack = new Stack<ThreadContainer>();
    	stack.push(this);
    	
    	while( !stack.isEmpty() ) {

    		ThreadContainer threadContainer = stack.pop();
    		
        	if (threadContainer.parent != null && threadContainer.threadable == null)
        		throw new RuntimeException("no threadable in " + threadContainer.toString());
        	threadContainer.parent = null;
            
        	if (threadContainer.threadable != null)
            	threadContainer.threadable.setNext(threadContainer.next == null ? null : threadContainer.next.threadable);

        	if (threadContainer.next != null) {
        		stack.push(threadContainer.next);
                threadContainer.next = null;
            }

        	if (threadContainer.threadable != null)
            	threadContainer.threadable.setChild(threadContainer.child == null ? null : threadContainer.child.threadable);
            
        	if (threadContainer.child != null) {
        		stack.push(threadContainer.child);
