...
public class DependencySimilarityCalculatorTest {
	@Autowired
	private ArtifactRepository artifactRepository;
	@Autowired
	@Qualifier("Dependency")
	private ISingleSimilarityCalculator depSim;
	private static final Logger logger = LoggerFactory.getLogger(DependencySimilarityCalculatorTest.class);
	...
	public void crossSimCommutativeTest() {
		double val1 = depSim.calculateSimilarity(artifact1, artifact2);
		double val2 = depSim.calculateSimilarity(artifact2, artifact1);
		logger.info(val1 + " " + val2);
		assertEquals(val1,val2, 0.0);
	}
	@Test
	public void crossSimIdentityTest() {
		double val1 = depSim.calculateSimilarity(artifact1, artifact1);
