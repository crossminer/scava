...
public class CompoundSimilarityCalculatorTest {
	@Autowired
	...
	private static final Logger logger = LoggerFactory.getLogger(CompoundSimilarityCalculatorTest.class);
	...
	public void crossSimCommutativeTest() {
		double val1 = compoundSim.calculateSimilarity(artifact1, artifact2);
		double val2 = compoundSim.calculateSimilarity(artifact2, artifact1);
		logger.info(val1 + " " + val2);
		assertEquals(val1,val2, 0.0);
	}
	@Test
	public void crossSimIdentityTest() {
		double val1 = compoundSim.calculateSimilarity(artifact1, artifact1);
