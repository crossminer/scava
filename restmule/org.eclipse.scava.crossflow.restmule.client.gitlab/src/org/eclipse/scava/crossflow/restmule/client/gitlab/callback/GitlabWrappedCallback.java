package org.eclipse.scava.crossflow.restmule.client.gitlab.callback;
...
public class GitlabWrappedCallback<D,R extends IWrap<D>> extends AbstractWrappedCallback<D,R, GitlabDataSet<D>>  {

	private static final Logger LOG = LogManager.getLogger(GitlabWrappedCallback.class);
	
	private static GitlabPagination paginationPolicy = GitlabPagination.get();
	
	public GitlabWrappedCallback() {
		super(new GitlabDataSet<D>());
	}
	...
	public Integer totalIterations(Response<R> response) { // FIXME
		...
		String pagination = GitlabPropertiesUtil.get(PAGE_INFO);
		String headerValue;
